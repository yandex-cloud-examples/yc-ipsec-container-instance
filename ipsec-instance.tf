...
// to remote subnets via IPsec Container Instance (inside interface)
...
// Reserve a static IP for the outside interface of IPsec Container Instance
...
// Create Security Group for outside interface of IPsec Container Instance
resource "yandex_vpc_security_group" "instance_sg" {
  folder_id   = var.folder_id
  name        = var.sg_name
  description = "IPsec Container Instance outside interface SG"
...
// Create IPsec Container Instance VM
